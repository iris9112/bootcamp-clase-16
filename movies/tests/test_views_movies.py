...
class MoviesViewsTest(TestCase):
    ...
    def setUp(self):
        test_user = User.objects.create_user(username='test_user', password='12345', is_superuser=True, is_staff=True)
        test_user.save()
    ...
    def test_view_contains_title_of_movie(self):
        ...
        response = self.client.get('/movies')
        self.assertContains(response, movie.__str__())
        self.assertEqual(response.status_code, 200)
